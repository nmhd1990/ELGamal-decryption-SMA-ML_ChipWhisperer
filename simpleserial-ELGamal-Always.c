...
/* Performs PART of a ELGAMAL decryption by using ****Square-and-Multiply Always Exponentiation Algorithm**** which uses only 16 bytes of keying material, where the "key" is
	...
	for(i = 0; i < 16; i++) {
		...
		for(j=8; j > 0; --j){
			...
			if(flag){ 
				...
				bigint_mul_u(&fake, &res, &base);
				bigint_reduce(&fake, r);
				
				if(t & (1<<(8-1))){
                bigint_copy(&res, &fake);
				}
			}
			t<<=1;
		}
